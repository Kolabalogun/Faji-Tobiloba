...
const NewProject = ({ handleState }) => {
  ...
  const { title, imgUrl, imgUrlI, imgUrlII, imgUrlIII, description } = form;

  const editorRefProjectDescription = useRef(null);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      title &&
      file &&
      editorRefProjectDescription.current.getContent().length > 1
    ) {
      setloader(true);
      try {
        await addDoc(collection(db, "Projects"), {
          ...form,
          description: editorRefProjectDescription.current.getContent(),
  ...
  const updateProject = async (e) => {
    ...
    if (title && editorRefProjectDescription.current.getContent().length > 1) {
      setloader(true);
      ...
      <div className="mx-3">
        <div className="flex  flex-col  my-[10px]  text-[14px]">
          ...
          <Editor
            ...
            onInit={(evt, editor) =>
              (editorRefProjectDescription.current = editor)
            }
